...
get_access_token() {
    local access_token=$(curl -k -d "grant_type=password&username=$username&password=admin&scope=apim:api_view apim:api_publish apim:api_create apim:subscribe" -u $client_credentials https://localhost:8243/token | jq -r '.access_token')
    echo $access_token
}
